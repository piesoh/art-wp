...
function hatch_theme_setup() {
	...
	add_image_size( 'miniature-slide', 800, 625, false );
	add_image_size( 'miniature-article', 200, 160, false );
	add_image_size( 'miniature-galerie', 100, 80, false );
...
function hatch_image_sizes() {
	...
	add_image_size( 'archive-thumbnail', 220, 150, false );
	add_image_size( 'single-thumbnail', 640, 500, false );
}
...
function hatch_commentform_args( $args ) {
	...
	$args = array(
		...
		'id_form' => 'commentform',
		'id_submit' => 'submit',
		'title_reply' => __( 'Laisser un message', 'hatch' ),
		'title_reply_to' => __( 'Répondre à %s', 'hatch' ),
...
function hatch_site_title() {
	...
	if ( hybrid_get_setting( 'hatch_logo_url' ) ) {	
		...
		echo '<' . $tag . ' id="site-title">' . "\n";
			...
			echo '</a>' . get_bloginfo( 'description' )."\n";
		echo '</' . $tag . '>' . "\n";
